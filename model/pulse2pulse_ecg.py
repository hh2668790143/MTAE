...
class WaveGANGenerator(nn.Module):
    def __init__(self, list_upsample,model_size=50, ngpus=1, num_channels=8,
                 # latent_dim=100,
        ...
        if upsample:
            stride = 1
            upsample = 5
        self.deconv_1 = Transpose1dLayer(5 * model_size, 5 * model_size, 25, stride, upsample=list_upsample[0])
        self.deconv_2 = Transpose1dLayer_multi_input(5 * model_size * 2, 3 * model_size, 25, stride, upsample=list_upsample[1])
        ...
        self.deconv_6 = Transpose1dLayer_multi_input(int(model_size / 2) * 2, int(model_size / 5), 25, stride,
                                                     upsample=upsample)
        ...
        self.classifier = torch.nn.Sequential(
            ...
            # torch.nn.Linear(opt.nz, opt.noisy_classify),
            # torch.nn.Softmax()
        )
    ...
    def forward(self, x, y):
        ...
        conv_1_out_x = F.leaky_relu(self.conv_1(x))  # x = (bs, 10, 500)
        # print("conv_1_out shape:", conv_1_out.shape)
        conv_2_out_x = F.leaky_relu(self.conv_2(conv_1_out_x))  # (25,100)
        # print("conv_2_out shape:", conv_2_out.shape)
        conv_3_out_x = F.leaky_relu(self.conv_3(conv_2_out_x))  # (50,50)
        # print("conv_3_out shape:", conv_3_out.shape)
        conv_4_out_x = F.leaky_relu(self.conv_4(conv_3_out_x))  # (150,10)
        # print("conv_4_out shape:", conv_4_out.shape)
        conv_5_out_x = F.leaky_relu(self.conv_5(conv_4_out_x))  # (250,2)

        conv_1_out_y = F.leaky_relu(self.conv_1(y))  # x = (bs, 10, 500)
        # print("conv_1_out shape:", conv_1_out.shape)
        conv_2_out_y = F.leaky_relu(self.conv_2(conv_1_out_y))  # (25,100)
        # print("conv_2_out shape:", conv_2_out.shape)
        conv_3_out_y = F.leaky_relu(self.conv_3(conv_2_out_y))  # (50,50)
        # print("conv_3_out shape:", conv_3_out.shape)
        conv_4_out_y = F.leaky_relu(self.conv_4(conv_3_out_y))  # (150,10)
        # print("conv_4_out shape:", conv_4_out.shape)
        conv_5_out_y = F.leaky_relu(self.conv_5(conv_4_out_y))  # (250,2)

        # print("conv_5_out shape:", conv_5_out.shape)
        x = F.leaky_relu(self.conv_6(conv_5_out_x))  # (250,1)
        y = F.leaky_relu(self.conv_6(conv_5_out_y))  # (250,1)

        x = F.relu(self.deconv_1(x))  # (250,5)
        if self.verbose:
            print(x.shape)
        x = F.relu(self.deconv_2(x, conv_5_out_x))
        if self.verbose:
            print(x.shape)
        x = F.relu(self.deconv_3(x, conv_4_out_x))
        if self.verbose:
            print(x.shape)
        x = F.relu(self.deconv_5(x, conv_3_out_x))
        if self.verbose:
            print(x.shape)
        x = F.relu(self.deconv_6(x, conv_2_out_x))
        if self.verbose:
            print(x.shape)
        rec_x = torch.tanh(self.deconv_7(x))

        if self.verbose:
            print(rec_x.shape)

        cls_y = self.classifier(y)
        return rec_x, x, y, cls_y

    # def forward(self, x):
    #     #x=(32,2,1000)
    #     #print("x shape:", x.shape)
    #     conv_1_out = F.leaky_relu(self.conv_1(x)) # x = (bs, 10, 500)
    #    # print("conv_1_out shape:", conv_1_out.shape)
    #     conv_2_out = F.leaky_relu(self.conv_2(conv_1_out))  #(25,100)
    #    # print("conv_2_out shape:", conv_2_out.shape)
    #     conv_3_out = F.leaky_relu(self.conv_3(conv_2_out))  #(50,50)
    #    # print("conv_3_out shape:", conv_3_out.shape)
    #     conv_4_out = F.leaky_relu(self.conv_4(conv_3_out))  #(150,10)
    #    # print("conv_4_out shape:", conv_4_out.shape)
    #     conv_5_out = F.leaky_relu(self.conv_5(conv_4_out)) #(250,2)
    #     #x = F.leaky_relu(self.conv_5(conv_4_out)) #(150,2)
    #
    #    #print("conv_5_out shape:", conv_5_out.shape)
    #     x = F.leaky_relu(self.conv_6(conv_5_out))  #(250,1)
...
class WaveGANDiscriminator(nn.Module):
    ...
    def forward(self, x):
        x = F.leaky_relu(self.conv1(x), negative_slope=self.alpha)  # (50 , 2499) (50,499)
        if self.verbose:
            print(x.shape)
        x = self.ps1(x)

        x = F.leaky_relu(self.conv2(x), negative_slope=self.alpha)  # (100, 1249)  (100,249)
        if self.verbose:
            print(x.shape)
        x = self.ps2(x)

        x = F.leaky_relu(self.conv3(x), negative_slope=self.alpha)  # (250, 624)  (250,124)
        if self.verbose:
            print(x.shape)
        x = self.ps3(x)

        x = F.leaky_relu(self.conv4(x), negative_slope=self.alpha)  # (500 , 311)  (500,61)
        if self.verbose:
            print(x.shape)
        x = self.ps4(x)

        x = F.leaky_relu(self.conv5(x), negative_slope=self.alpha)  # (1000, 78)  (1000,15)
        if self.verbose:
            print(x.shape)
        x = self.ps5(x)

        x = F.leaky_relu(self.conv6(x), negative_slope=self.alpha)  # (1250, 19)  (1250,4)
        if self.verbose:
            print(x.shape)

        x = F.leaky_relu(self.conv7(x), negative_slope=self.alpha)  # (5000,5)
        if self.verbose:
            print(x.shape)

        x = x.view(-1, x.shape[1] * x.shape[2])  # (128,5000)
        if self.verbose:
            print(x.shape)
...
# x = Variable(torch.randn(32, 1, 1000))
# G = WaveGANGenerator(num_channels=1,upsample=True,list_upsample=[2,5])
# out = G(x,x)
# print(out.shape)
...
# netG = WaveGANGenerator(num_channels=2, model_size=opt.model_size, ngpus=opt.ngpus, upsample=True)
