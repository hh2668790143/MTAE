...
class WaveGANGenerator(nn.Module):
    ...
    def forward(self, x):
        ...
        conv_1_out = F.leaky_relu(self.conv_1(x)) # x = (bs, 10, 2500)
       # print("conv_1_out shape:", conv_1_out.shape)
        conv_2_out = F.leaky_relu(self.conv_2(conv_1_out)) #(25, 500)
       # print("conv_2_out shape:", conv_2_out.shape)
        conv_3_out = F.leaky_relu(self.conv_3(conv_2_out)) #(50,250)
       # print("conv_3_out shape:", conv_3_out.shape)
        conv_4_out = F.leaky_relu(self.conv_4(conv_3_out)) #(150, 50)
       # print("conv_4_out shape:", conv_4_out.shape)
        conv_5_out = F.leaky_relu(self.conv_5(conv_4_out))  #(250,10)
       # print("conv_5_out shape:", conv_5_out.shape)
        x = F.leaky_relu(self.conv_6(conv_5_out))   #(250,2)
        ...
        x = F.relu(self.deconv_1(x))  #(250,10)
        if self.verbose:
            print(x.shape)

        x = F.relu(self.deconv_2(x, conv_5_out))  #(150, 50)
        if self.verbose:
            print(x.shape)
        ...
        x = F.relu(self.deconv_6(x, conv_2_out))  #(10,2500)
        if self.verbose:
            print(x.shape)

        output = torch.tanh(self.deconv_7(x)) #(8,5000)
...
class WaveGANDiscriminator(nn.Module):
    ...
    def forward(self, x):
        x = F.leaky_relu(self.conv1(x), negative_slope=self.alpha)  #(50 , 2499)
        if self.verbose:
            print(x.shape)
        x = self.ps1(x)

        x = F.leaky_relu(self.conv2(x), negative_slope=self.alpha)  #(100, 1249)
        if self.verbose:
            print(x.shape)
        x = self.ps2(x)

        x = F.leaky_relu(self.conv3(x), negative_slope=self.alpha)  #(250, 624)
        if self.verbose:
            print(x.shape)
        x = self.ps3(x)

        x = F.leaky_relu(self.conv4(x), negative_slope=self.alpha)  #(500 , 311)
        if self.verbose:
            print(x.shape)
        x = self.ps4(x)

        x = F.leaky_relu(self.conv5(x), negative_slope=self.alpha) #(1000, 78)
        if self.verbose:
            print(x.shape)
        x = self.ps5(x)

        x = F.leaky_relu(self.conv6(x), negative_slope=self.alpha) #(1250, 19)
        if self.verbose:
            print(x.shape)
        x = self.ps6(x)
...
if __name__ == '__main__':
    ...
    x = Variable(torch.randn(1, 8, 2000))
    G = WaveGANGenerator(verbose=True, upsample=True)
    out = G(x)
    print(out.shape)
