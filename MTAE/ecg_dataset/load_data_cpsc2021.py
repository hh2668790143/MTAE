...
def get_data(root):
    normal_path = glob.glob(root + "*_normal.pickle")
    fangchan_path = glob.glob(root + "*_fangchan.pickle")
    # 获得患者id
...
def load_data_cpsc2021(root, len_num=1000, seed=1024):
    ...
    for i in all_normal_patient[:28]:  # 41
        normal_data.append(i[:, :, :len_num])

    for i in all_normal_abnormal_patient_N[:3]:  # 41
        normal_data.append(i[:, :, :len_num])

    for i in all_abnormal_patient[:7]:  # 31
        abnormal_data.append(i[:, :, :len_num])

    for i in all_normal_abnormal_patient_A[:1]:  # 31
        abnormal_data.append(i[:, :, :len_num])
    ...
    abnormal_len = len(abnormal_data)

    train_data = normal_data[0:normal_len * 2 // 3]

    val_normal_data = normal_data[normal_len * 2 // 3:normal_len * 5 // 6]
    val_abnormal_data = abnormal_data[0:abnormal_len // 2]

    test_normal_data = normal_data[normal_len * 5 // 6:]
    test_abnormal_data = abnormal_data[abnormal_len // 2:]
...
def load_data_cpsc2021_cls(root, len_num=1000, seed=1024):
    all_normal_patient, all_abnormal_patient, \
        all_normal_abnormal_patient_N, all_normal_abnormal_patient_A = get_data(root)
    # 51 normal
    # 23 abnormal
    ...
    for i in all_normal_abnormal_patient_A[:23]:  # 23
        ...
        np.random.seed(seed)
        np.random.shuffle(i)
        train_abnormal_data.append(i[100:200, :, :len_num])  # 100

    train_abnormal_data = np.concatenate(train_abnormal_data, axis=0)
    train_abnormal_data = train_abnormal_data[100:101, :, :]

    return train_data, train_abnormal_data, val_normal_data, val_abnormal_data, test_normal_data, test_abnormal_data


def load_data_cpsc2021_p(root, len_num=1000, seed=1024):
    all_normal_patient, all_abnormal_patient, \
    all_normal_abnormal_patient_N, all_normal_abnormal_patient_A = get_data(root)

    all_normal_patient = all_normal_patient[:, :, :len_num]
    all_abnormal_patient = all_abnormal_patient[:, :, :len_num]
    all_normal_abnormal_patient_N = all_normal_abnormal_patient_N[:, :, :len_num]
    all_normal_abnormal_patient_A = all_normal_abnormal_patient_A[:, :, :len_num]
